...
def q_learning( env,
    ...
    for ep in range(max_episodes):
        ...
        for t in itertools.count():
            ...
            td_target = reward + discount_factor * np.max(estimator.v(next_state))
            estimator.update(state, action, td_target)

            # Remember experience if estimator has remember
            estimator.remember(state, action, reward, next_state, done)
            
            if done:
                break
                
            # Experience a replay if estimator has replay
            if total_t % learn_every == 0:
                estimator.replay(batch_size, discount_factor)

            # Update target network if estimator has update
