import pytest
import math
import project.environment.boosterbooster as booster

class Testbooster(object):
    ...
    def test_engine_impulse_90_degree_orientation(self):
        Fx, Fy = booster.engine_impulse(500, 0, 0, math.pi/2)
        expected_Fx = 500
        expected_Fy = 0
        assert round(Fx,5) == expected_Fx and round(Fy,5) == expected_Fy

    def test_engine_impulse_90_degree_orientation_with_dispersion(self):
        Fx, Fy = booster.engine_impulse(500, 0, 0.1, math.pi/2)
        expected_Fx = 497.50208
        expected_Fy = -49.91671
        assert round(Fx,5) == expected_Fx and round(Fy,5) == expected_Fy
    
    def test_engine_impulse_edge(self):
        Fx, Fy = booster.engine_impulse(0, math.pi, math.pi, math.pi)
        expected_Fx = 0
        expected_Fy = 0
        assert round(Fx,5) == expected_Fx and round(Fy,5) == expected_Fy
    ...
    def test_side_engine_impulse_position_left_thruster(self, body):
        impulse_position = booster.side_engine_impulse_position(body, -1, 40, 3)
    ...
    def test_side_engine_impulse_position_edge(self, body):
        impulse_position = booster.side_engine_impulse_position(body, 1, 0, 0)
