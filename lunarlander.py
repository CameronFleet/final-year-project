...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(self):
        ...
        W = config.VIEWPORT_W / config.SCALE
        H = config.VIEWPORT_H / config.SCALE
    ...
    def step(self, action):
        ...
        if (self.continuous and action[0] > 0.0) or (not self.continuous and action == 2):
            ...
            if self.continuous:
                ...
                m_power = 1.0
            ox = tip[0] * (4 / config.SCALE + 2 * dispersion[0]) + side[0] * dispersion[
                1]  # 4 is move a bit downwards, +-2 for randomness
            oy = -tip[1] * (4 / config.SCALE + 2 * dispersion[0]) - side[1] * dispersion[1]
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (not self.continuous and action in [1, 3]):
            ...
            if self.continuous:
                ...
                direction = action - 2
                s_power = 1.0
            ox = tip[0] * dispersion[0] + side[0] * (3 * dispersion[1] + direction * config.SIDE_ENGINE_AWAY / config.SCALE)
            oy = -tip[1] * dispersion[0] - side[1] * (3 * dispersion[1] + direction * config.SIDE_ENGINE_AWAY / config.SCALE)
            impulse_pos = (self.lander.position[0] + ox - tip[0] * 17 / config.SCALE,
                           self.lander.position[1] + oy + tip[1] * config.SIDE_ENGINE_HEIGHT / config.SCALE)
        ...
        state = [
            (pos.x - config.VIEWPORT_W / config.SCALE / 2) / (config.VIEWPORT_W / config.SCALE / 2),
            (pos.y - (self.helipad_y + config.LEG_DOWN / config.SCALE)) / (config.VIEWPORT_H / config.SCALE / 2),
            vel.x * (config.VIEWPORT_W / config.SCALE / 2) / config.FPS,
            vel.y * (config.VIEWPORT_H / config.SCALE / 2) / config.FPS,
            self.lander.angle,
